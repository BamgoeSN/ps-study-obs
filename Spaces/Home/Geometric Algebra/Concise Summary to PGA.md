...
- Bivector의 exponential form은 Taylor series로 정의된다.
	- 직선을 의미하며 서로 수직인 두 unit vector $\hat{u}$, $\hat{v}$에 대해 $e^{\hat{u} \hat{v} \theta} := \cos\theta + \hat{u}\hat{v}\sin\theta$로 정의된다. 이는 $i = \hat{u}\hat{v}$라 하면 $i^2 = -1$이라는 점에서 Euler's formula와 통하는 면이 있다.
